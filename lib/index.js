...
internals.determineTandy = (route, thisRouteOpts) => {
    ...
    //count is only allowed at the end of a path
    if (path.indexOf('count') !== -1 &&
        pathInfo.segments.indexOf({ literal: 'count' }) !== pathInfo.segments.length - 1) {

        throw new Error('Count can only appear at the end of a route path');
    }
